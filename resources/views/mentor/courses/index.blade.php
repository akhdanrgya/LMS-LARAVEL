                    ...
                    <tr class="border-b border-gray-200 hover:bg-gray-100 transition-colors duration-150">
                        <td class="py-3 px-6 text-left">
                            @if($course->thumbnail_path)
                                <img src="{{ $course->thumbnail_url }}" alt="{{ $course->title }}" class="w-20 h-12 object-cover rounded-md">
                            @else
                                <div class="w-20 h-12 bg-gray-300 flex items-center justify-center text-xs text-gray-500 rounded-md">No Image</div>
                            @endif
                        </td>
                        ...
                        <td class="py-3 px-6 text-center">
                            <div class="flex item-center justify-center space-x-2">
                                {{-- Tombol Kelola Materi/Quiz (Placeholder untuk nanti) --}}
                                {{-- <a href="#" class="w-8 h-8 rounded bg-blue-500 text-white flex items-center justify-center transform hover:scale-110" title="Kelola Konten">
                                    <i class="fas fa-list-ul"></i>
                                </a> --}}
                                <a href="{{ route('mentor.courses.edit', $course->slug) }}" class="w-8 h-8 rounded bg-yellow-500 text-white flex items-center justify-center transform hover:scale-110" title="Edit Course">
                                    <i class="fas fa-pencil-alt"></i>
                                </a>
                                <form action="{{ route('mentor.courses.destroy', $course->slug) }}" method="POST" onsubmit="return confirm('Yakin mau hapus course \'{{ $course->title }}\'? Semua materi dan quiz di dalamnya juga akan terhapus!');" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-8 h-8 rounded bg-red-500 text-white flex items-center justify-center transform hover:scale-110" title="Hapus Course">

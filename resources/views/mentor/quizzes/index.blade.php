                    ...
                    <tr class="border-b hover:bg-gray-100">
                        ...
                        <td class="py-3 px-6 text-center">
                            <div class="flex item-center justify-center space-x-2">
                                <a href="{{-- route('mentor.courses.quizzes.questions.index', [$course->slug, $quiz->id]) --}}" class="text-blue-500 hover:text-blue-700" title="Kelola Pertanyaan">
                                    <i class="fas fa-tasks"></i> Soal
                                </a>
                                <a href="{{ route('mentor.courses.quizzes.edit', [$course->slug, $quiz->id]) }}" class="text-yellow-500 hover:text-yellow-700" title="Edit Quiz">
                                    <i class="fas fa-pencil-alt"></i>
                                </a>
                                <form action="{{ route('mentor.courses.quizzes.destroy', [$course->slug, $quiz->id]) }}" method="POST" onsubmit="return confirm('Yakin hapus quiz ini? Semua pertanyaan di dalamnya juga akan hilang.');" class="inline-block">
                                    @csrf
                                    @method('DELETE')

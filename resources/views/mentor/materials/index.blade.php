                            ...
                            <tr class="border-b border-gray-200 hover:bg-gray-100 transition-colors duration-150">
                                ...
                                <td class="py-3 px-6 text-left">
                                    @if($material->content_type == 'text')
                                        <span
                                            class="px-2 py-1 text-xs font-semibold leading-tight text-blue-700 bg-blue-100 rounded-full">Teks/Artikel</span>
                                    @elseif($material->content_type == 'video_link')
                                        <span
                                            class="px-2 py-1 text-xs font-semibold leading-tight text-purple-700 bg-purple-100 rounded-full">Link
                                            Video</span>
                                    @elseif($material->content_type == 'file_path')
                                        <span
                                            class="px-2 py-1 text-xs font-semibold leading-tight text-green-700 bg-green-100 rounded-full">File</span>
                                    @else
                                        <span
                                            class="px-2 py-1 text-xs font-semibold leading-tight text-gray-700 bg-gray-100 rounded-full">{{ ucfirst(str_replace('_', ' ', $material->content_type)) }}</span>
                                    @endif
                                </td>
                                <td class="py-3 px-6 text-center">
                                    <div class="flex item-center justify-center space-x-2">
                                        ...
                                        {{-- <a href="#"
                                            class="w-8 h-8 rounded bg-sky-500 text-white flex items-center justify-center transform hover:scale-110"
                                            title="Lihat Materi">
                                            <i class="fas fa-eye"></i>
                                        </a> --}}
                                        {{-- Di kolom Aksi tabel course --}}
                                        <a href="{{ route('mentor.courses.quizzes.index', $course->slug) }}"
                                            class="w-8 h-8 rounded bg-purple-500 text-white flex items-center justify-center transform hover:scale-110"
                                            title="Kelola Quiz">
                                            <i class="fas fa-puzzle-piece"></i> {{-- Atau ikon lain buat quiz --}}
                                        </a>
                                        <a href="{{ route('mentor.courses.materials.edit', ['course' => $course->slug, 'material' => $material->id]) }}"
                                            class="w-8 h-8 rounded bg-yellow-500 text-white flex items-center justify-center transform hover:scale-110"
                                            title="Edit Materi">
                                            <i class="fas fa-pencil-alt"></i>
                                        </a>
                                        <form
                                            action="{{ route('mentor.courses.materials.destroy', ['course' => $course->slug, 'material' => $material->id]) }}"
                                            method="POST"
                                            onsubmit="return confirm('Yakin mau hapus materi \'{{ $material->title }}\'?');"
                                            class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="w-8 h-8 rounded bg-red-500 text-white flex items-center justify-center transform hover:scale-110"
                                                title="Hapus Materi">
